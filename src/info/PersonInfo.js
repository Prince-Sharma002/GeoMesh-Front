...
const PersonInfo = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        ...
        const polygonResponse = await axios.get(`https://geomesh-back-8gtx.onrender.com/api/polygons/email?email=${email}`);
  ...
  const handleDelete = async (polygonId) => {
    try {
      const response = await axios.delete(`https://geomesh-back-8gtx.onrender.com/api/polygon/${polygonId}`);
